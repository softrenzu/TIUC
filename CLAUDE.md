...
- フロントは `public/` 配下の素の HTML + ES モジュール。ビルド工程なし。
  - `public/index.html` … トップページ = キャラ育成ゲーム「モリノミカタ」。全員が最初に
    通るページで、通報・トリアージ投票は「相棒(在来種側、暫定でキツツキ)を育てるアクション」
    として導線を張る。「ヨソモン」は本アプリ内では一貫して外来種(クビアカツヤカミキリ)
    側の呼称としてのみ使う(相棒側の呼称ではない。旧仕様からの呼称変更 — 旧版では
    「ヨソモン」がキャラ育成ゲームのブランド名だったが、意味が逆(害虫側)なので誤解を招く
    ため2026-07に是正した)。
...
3. **ゲーミフィケーションは水増しを構造で防ぐ。**
   ...
   - `trust_score` は確定率のラプラス平滑化。レビュー優先度と表示順に反映。ハンターランク
     (見習い/一人前/ベテラン/エキスパート、`hunterRank()`)の唯一の入力としても再利用する
     (新規の計算ロジックは持たない。`trust_score`と`submitted_count`の両方が閾値を超えないと
     昇格しない — 少数サンプルの偶然の高スコアだけで最上位に届かないようにするため)。
   - キャラ育成ゲーム「モリノミカタ」(`index.html`)のXP(通報送信・トリアージ投票の
     どちらでも貯まる)は、この経済と完全に分離した別台帳(`xp_events`/`characters`)で持つ。
     「投稿の正確性」に紐づく既存ポイントの意味を、育成要素と混ぜて壊さないため。
   - XP側も「量でなく正しさ」を追加で徹底する: 通報時のXPは小さく、レビュアーが「被害あり」
     確定した時のみ大きい`confirm_bonus`(+15XP)を追加で払う。「該当なし」確定時は投稿時に
     払ったXPを`revoke`(実際に払った額の実費、`SUM(xp_events.amount WHERE kind=
     'report_submit')`で都度算出)で取り消す。取り消しでレベルが下がることもあり得るが、
     これは意図した挙動として許容する。
   - 同一メッシュ(mesh5)への1時間以内の連続投稿はXPを半分に割り引く(スパム抑止。
     `auto_rejected`の投稿も連投カウントには含める — 本物とゴミを交互に送って抑止を
     回避させないため)。
   - 定点観測ストリーク: 同じ通報者が同じmesh5に20時間以上・14日以内の間隔で再訪すると
     連続回数が伸び、XPにボーナスが乗る(`Math.min(streak_count, 5)`を加算)。別のmeshへ
     移動するか間隔が空きすぎるとリセットされる。ボーナス額は独立した`xp_events`行にせず
     `report_submit`の`amount`に合算する(revoke時に「この投稿で払った合計」を1回の
     `SUM`で正確に取り消せるようにするため)。
...
`xp_events`(通報送信・トリアージ投票・クエスト報酬・確定ボーナス・取り消しのXP台帳。
`point_events`とは完全に別経済 — rule3参照。`kind`: `triage_vote`/`report_submit`/
`quest_bonus`/`confirm_bonus`/`revoke`)、`quest_claims`(クエスト受け取り済みフラグのみ。
達成判定自体はクライアントを信用せずサーバ側で毎回`reports`/`triage_votes`の実データから
再評価する。クエスト内容そのものはDB駆動ではなく`src/index.js`の`QUESTS`定数で管理)。
`reporters`には定点観測ストリーク用の`streak_mesh5`/`streak_count`/`streak_at`列がある
(`migrations/0007_quests_streaks.sql`)。図鑑(発見物×樹種の収集要素)は専用テーブルを持たず
`reports`から直接集計する(`GET /api/encyclopedia`)。
...
完了: 通報フォーム(地図タップでの位置指定フォールバック込み) / 通報受付 API / R2保存 / D1登録 /
      ...
      変わる節目(Lv3・Lv6・Lv10)では「進化した!」の特別演出になる(Canvas紙吹雪 +
      Web Audio APIのオシレーターで合成したチャイム音 + `navigator.vibrate()`。
      音声・画像アセットは追加していない)。レベルアップモーダルには「📤 シェア」ボタンがあり、
      `<canvas>`でLv.・絵文字を描画したPNGを`navigator.share()`(非対応時はダウンロード
      リンク)でシェアできる。/
      図鑑(`GET /api/encyclopedia`、`mypage.html`): 発見物4種(フラス/成虫生/成虫死/脱出孔。
      「異常なし」は不在データなので対象外)×樹種5種(サクラ/ウメ/モモ/スモモ/その他バラ科。
      unknown/バラ科以外は対象外)= 20枠を`reports`から直接集計。`status='auto_rejected'`は
      収集対象から除外(ゴミ写真の水増し防止、rule3)。/
      クエスト(`GET /api/quests`・`POST /api/quests/claim`、`mypage.html`): 固定5種
      (はじめての記録・3樹種チャレンジ・判定マイスター・夏の成虫パトロール(6〜8月の季節限定、
      クビアカの成虫発生期に合わせた実際の生態ベース)・図鑑コンプリート)。二重受給は
      `quest_claims`の複合主キー + `changes()`ゲートのSQLで防止。/
      地域インパクト・ローカルランキング(`GET /api/impact`、`mypage.html`「地域のようす」):
      `survey_mesh`が未整備のためmesh3(約1km四方)を「地域」の代用単位とする。今月のその
      mesh3内の件数・確定件数と、自分の順位/総参加者数のみを返す(他人の識別情報は一切
      返さない。rule1に抵触しない設計)。/
      ハンターランク(`trust_score`ベース、`hunterRank()`)・定点観測ストリークの表示を
      `mypage.html`の統計欄に追加。/
      デザイントークン(`public/tokens.css`、CSS変数。`index.html`/`report.html`/
      `mypage.html`のみ読み込み、他ページは対象外)を新設し、UIを刷新: `report.html`は
      5ステップ(写真→発見物→樹種→位置→メモ)のウィザードに再構成(進捗バー付き。
      既存のEXIF解析・近隣重複確認・AI却下フロー・地図フォールバックのロジックは変更せず
      見た目の並び方のみ変更)、発見物の選択肢に絵文字アイコンを追加。`report.html`/
      `mypage.html`には`/api/character`から取得する小さな「相棒バッジ」(Lv.+絵文字)を
      常駐表示。`mypage.html`は通報0件時のCTA付き空状態、`response_updated_at`が直近30日
      以内の通報カードに「🔔 更新あり」バッジを追加。
未着手: チーム機能(参加・作成・管理UI) / Cloudflare Access 移行 / 独自ドメイン /
        キャラ育成ゲームの進化・バトル要素(仕様未定) / プッシュ通知・メール通知基盤
        (`response_note`をマイページで見せる以上の能動的な通知は無い)。
