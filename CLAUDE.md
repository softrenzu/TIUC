...
1. **座標の出し分け(最重要)**
   ...
   - `/img/` 配下の画像配信はレビュアー認証必須(私有地の写真のため)。
...
完了: 通報フォーム / 通報受付 API / R2保存 / D1登録 / レビュー画面(判定・土地種別・対応状況・お知らせ)。
未着手: **公開マップ**(次の作業) / スタンプ / AI一次判定 / 未調査メッシュ表示 / チーム機能 /
        通報者マイページ / Cloudflare Access 移行 / 独自ドメイン。

既知の詰まり: ローカル(PC)の navigator.geolocation が NG1(拒否)で通らない。
  → アプリの問題ではなく PC 環境の問題。本番のスマホでは動く。
  → 恒久対策として「地図タップで場所を指定」する導線を公開マップ実装時に入れる予定
    (GPS 不許可の人・後から自宅で報告する人にも必要)。

## 次の作業: 公開マップ

- MapLibre GL JS(CDN)+ 国土地理院 淡色地図タイル(出典表示で無料・商用可)。
- ズームで粒度切替(広域=1kmメッシュ、詳細=250mメッシュ)。メッシュコードから矩形を逆算し GeoJSON 化
  (mesh.js に逆変換関数を追加)。
- 公有地の確定案件のみピン表示。私有地は絶対にピンにしない(メッシュ集計に含めるのみ)。
- フィルタ: 日付範囲 / ステータス(被害あり・対応中・対応済みなど)。
- API 例: `GET /api/map?bbox=...&from=...&status=...` が GeoJSON を返す。座標はメッシュ集計のみ。
  Cache API で数分キャッシュ可。
- 目的: 重複通報の抑制、「自分だけじゃない」という参加感、行政の対応状況の可視化。
